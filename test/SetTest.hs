import Set (contains, delete, filterSet, foldlSet, foldrSet, insert, listToSet, mapSet, mergeSets, setToList, sizeSet)
...
tests =
  TestList
    [ TestLabel "test insert delete" test1,
...
prop_insert_delete list elementToInsert = do
  ...
  if contains tree elementToInsert
    then tree == inserted
    else tree == delete elementToInsert inserted
...
prop_merge_assoc list1 list2 list3 = do
  ...
  mergeSets (mergeSets tree1 tree2) tree3 == mergeSets tree1 (mergeSets tree2 tree3)
